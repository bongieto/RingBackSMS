...
- **Double-parens phrasing: the first parens is usually just the item name.** Many menu items have parenthetical ingredient descriptions baked into their name (e.g. "Cornsilog (Corned Beef, Sinangag, Itlog)", "Tapsilog (Tapa, Sinangag, Itlog)"). When the customer writes "1 Cornsilog (Corned Beef, Sinangag, Itlog) (Extra Corned Beef)", the FIRST parens restates the item's own name; the SECOND parens is the actual modifier ("Extra Corned Beef"). Only emit modifiers for the latter. Same when only one parens group is present — if its contents overlap heavily with the item's own name, treat it as redundant and emit NO modifiers.
- For modifier group_name values: don't stress about matching group names exactly — our tool will fuzzy-match by modifier_name alone when the group is wrong. Pass your best guess for group_name (or an empty string) and the real modifier_name.
