...
export async function GET(request: NextRequest, { params }: { params: { tenantId: string } }) {
  ...
  const [totalMissedCalls, totalConversations, totalOrders, totalMeetings, recentUsage, orderRevenue] = await Promise.all([
  ...
  return apiSuccess({ period: { days, since }, missedCalls: totalMissedCalls, conversations: totalConversations, orders: totalOrders, meetings: totalMeetings, revenue, usage });
}
