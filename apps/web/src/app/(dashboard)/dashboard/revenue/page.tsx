...
import { DollarSign, ShoppingBag, TrendingUp, Clock, RotateCcw } from 'lucide-react';
...
export default function RevenuePage() {
  ...
  return (
    <div>
      <Header
        title="Revenue"
        description="Canonical paid sales, refunds, tickets, and item mix"
        action={
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
        <StatCard
          title="Net sales"
          value={fmtDollars(data?.totals.netCents ?? 0)}
          icon={DollarSign}
          iconColor="text-emerald-500"
        />
        <StatCard
          title="Gross sales"
          value={fmtDollars(data?.totals.grossCents ?? 0)}
          icon={TrendingUp}
          iconColor="text-cyan-500"
        />
        <StatCard
          title="Refunds"
          value={fmtDollars(data?.totals.refundCents ?? 0)}
          icon={RotateCcw}
        ...
        <StatCard
          title="Avg ticket"
          value={fmtDollars(data?.totals.avgTicketCents ?? 0)}
          icon={TrendingUp}
          iconColor="text-purple-500"
        />
        <StatCard
          title="Tips"
          value={fmtDollars(data?.totals.tipCents ?? 0)}
          icon={DollarSign}
